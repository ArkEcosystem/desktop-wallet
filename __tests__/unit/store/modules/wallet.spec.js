...
describe("WalletModule", () => {
	...
	describe("getters byProfileId", () => {
		...
		describe("when the profile has wallets", () => {
			it("should return them", () => {
				...
				expect(store.getters["wallet/byProfileId"]("exampleId")).not.toIncludeSameMembers(
					contactWalletsOfExampleId,
				);
			});
		});
	...
	describe("getters contactsByProfileId", () => {
		...
		describe("when the profile has wallets", () => {
			it("should return them", () => {
				const contactWalletsOfExampleId = [models[4], models[5]];
				const walletsOfExampleId = [models[0], models[3]];
				expect(store.getters["wallet/contactsByProfileId"]("exampleId")).toIncludeSameMembers(
					contactWalletsOfExampleId,
				);
				expect(store.getters["wallet/contactsByProfileId"]("exampleId")).not.toIncludeSameMembers(
					walletsOfExampleId,
				);
			});
		});
	...
	describe("getters publicByProfileId", () => {
		it.each(["address", "balance", "name", "publicKey", "vote"])(
			"should include wallet %s property",
			(property) => {
		...
		describe("when the session network is the network of the requested profile", () => {
			it("should include ledger wallets", () => {
				const walletsOfExampleId = [models[0], models[3], ledgerWallets.L1, ledgerWallets.L2].map(
		...
		describe("when the session network is not the network of the requested profile", () => {
			it("should not include ledger wallets", () => {
				...
				expect(store.getters["wallet/publicByProfileId"]("otherId")).not.toIncludeSameMembers(
					ledgerWalletsOfOtherId,
				);
			});
		});
	});
