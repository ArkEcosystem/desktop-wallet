...
import peers, { badPeer1, goodPeer1, goodPeer2, goodPeer4, goodPeer5 } from "../../__fixtures__/store/peer";
...
describe("peer store module", () => {
	...
	it("should refresh peer list for v2", async () => {
		...
		for (const peer of refreshPeers) {
			nock(`http://${peer.ip}:${peer.ports["@arkecosystem/core-api"]}`).persist().get("/api/peers").reply(200, {
				data: refreshPeers,
			});
		}
