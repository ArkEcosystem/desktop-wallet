	...
	describe("getters", () => {
		...
		describe("filtered", () => {
			describe.only("when given a filter", () => {
				beforeAll(() => {
					store.replaceState(
						merge(JSON.parse(JSON.stringify(initialState)), {
							plugin: {
								available: {
				...
				it("should filter official plugins only", () => {
					expect(
						store.getters["plugin/filtered"](null, null, "official").map((plugin) => {
				...
				it("should filter funded plugins only", () => {
					expect(
						store.getters["plugin/filtered"](null, null, "funded").map((plugin) => {
							return plugin.config.id;
						}),
		...
		describe("menuItems", () => {
			...
			it("should retrieve all menu items", () => {
				expect(store.getters["plugin/menuItems"]).toEqual([{ title: "menu-item-1" }, { title: "menu-item-2" }]);
			});
		});
		...
		describe("walletTabs", () => {
			...
			it("should return the wallet tabs", () => {
				const spy = jest
					.spyOn(pluginManager, "getWalletTabComponent")
					.mockImplementation(() => "tab-1-component");
		...
		describe("profileHasPluginOptions", () => {
			...
			describe("when profile id given", () => {
				it("should return true if there are options", () => {
					expect(
						store.getters["plugin/profileHasPluginOptions"](availablePlugins[0].config.id, profile1.id),
					).toBe(true);
				});

				it("should return false if there are options", () => {
					expect(store.getters["plugin/profileHasPluginOptions"]("plugin-no-options", profile1.id)).toBe(
						false,
					);
				});
			});
		...
		describe("pluginOptions", () => {
			...
			it("should return an empty object if there are options for the given profile", () => {
				expect(
					store.getters["plugin/pluginOptions"](availablePlugins[0].config.id, "profile-no-options"),
				).toEqual({});
			});
	...
	describe("actions", () => {
		...
		describe("deletePlugin", () => {
			...
			it("should return early if plugin is not installed", async () => {
				expect(await store.dispatch("plugin/deletePlugin", { pluginId: availablePlugins[1].config.id })).toBe(
					undefined,
				);
			});
			...
			it.each(profiles)("should disable the plugin and delete its options", async (profile) => {
				...
				await store.dispatch("plugin/deletePlugin", {
					pluginId: availablePlugins[0].config.id,
					removeOptions: true,
				});
