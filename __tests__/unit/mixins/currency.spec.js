	...
	describe("currency_format", () => {
		...
		describe("when the `currencyDisplay` option is provided", () => {
			it("should use it to display the currency", () => {
				...
				expect(format(amount, { currencyFrom: "network", currencyDisplay: "code" })).toEqual(
					"NET\xa09,835.387653",
				);
				expect(format(amount, { currencyFrom: "network", currencyDisplay: "symbol" })).toEqual(
					"×\xa09,835.387653",
				);
		...
		describe("when the `subunit` option is provided", () => {
			...
			describe("when the curreny is not the network currency", () => {
				it("should use it to display the currency, indepently of the `currencyDisplay` option", () => {
					...
					expect(format(amount, { currencyFrom: "network", currencyDisplay: "code", subunit: true })).toEqual(
						"netoshi\xa0983,538,765,300.00",
					);
					expect(format(amount, { currency: "NET", currencyDisplay: "symbol", subunit: true })).toEqual(
						"netoshi\xa0983,538,765,300.00",
					);
				});
			});
