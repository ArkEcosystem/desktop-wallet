...
describe("WalletTransactions", () => {
	...
	describe("methods", () => {
		...
		describe("getStoredTransactions", () => {
			const transactions = [{ type: 10 }, { type: 1 }, { type: 7 }, { type: 0 }];
		...
		describe("fetchTransactions", () => {
			const remoteTransactions = ["remote-1"];
			const localTransactions = ["local-1"];
			...
			it("should attempt to merge stored and remote transaction arrays", async () => {
				...
				expect(mergeTableMock).toHaveBeenCalledWith(
					remoteTransactions,
					localTransactions,
			...
			it('should output error if not "wallet not found"', async () => {
				...
				nock("http://localhost").get("/wallets/address-1/transactions").query(true).reply(500, {
					message: "oops",
				});
			...
			it('should not output error if "wallet not found"', async () => {
				...
				nock("http://localhost").get("/wallets/address-1/transactions").query(true).reply(404, {
		...
		describe("onPageChange", () => {
			it("should do nothing if currentPage has not changed", () => {
				...
				const spyTransactions = jest
					.spyOn(WalletTransactionsMixin.methods, "loadTransactions")
					.mockImplementation();
				createWrapper();
