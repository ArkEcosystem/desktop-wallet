...
describe('WalletHeadingInfo component', () => {
  ...
  it('should display the balance in the currency network', () => {
    ...
    const phantomBalance = wrapper.vm.currency_subToUnit(sampleWalletData.balance)
    const formattedBalance = wrapper.vm.currency_format(phantomBalance, { currencyFrom: 'network' })
  ...
  describe('when the session network has the market enabled', () => {
    ...
    it('should display the balance in the alternative currency too', () => {
      ...
      const phantomBalance = wrapper.vm.currency_subToUnit(sampleWalletData.balance)
      const formattedBalance = wrapper.vm.currency_format(phantomBalance * price, { currency: alternativeCurrency })
