...
const plugins = [{ id: "plugin-enabled" }, { id: "plugin-disabled" }, { id: "plugin-available" }];
		...
		mocks: {
			$store: {
				getters: {
					...
					"plugin/isInstalled": jest.fn(
						(pluginId) => pluginId === "plugin-enabled" || pluginId === "plugin-disabled",
					),
...
describe("PluginManagerTable", () => {
	...
	it("should emit sort-change event when clicking on a categories header", () => {
		wrapper
			.findAll("th")
			.filter((node) => node.text() === "PLUGIN_TABLE.CATEGORY")
			.at(0)
			.trigger("click");
	...
	it("should emit sort-change event when clicking on a status header", () => {
		wrapper
			.findAll("th")
			.filter((node) => node.text() === "PLUGIN_TABLE.STATUS")
			.at(0)
			.trigger("click");
		expect(wrapper.emitted("on-sort-change")).toBeTruthy();
	});
