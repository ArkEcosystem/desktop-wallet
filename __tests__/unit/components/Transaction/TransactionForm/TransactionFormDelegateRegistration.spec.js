	...
	describe("template", () => {
		...
		describe("next button", () => {
			...
			it("should be disabled if form is invalid", async () => {
				...
				expect(wrapper.find(".TransactionFormDelegateRegistration__next").attributes("disabled")).toBe(
					"disabled",
				);
			});
		});
	...
	describe("methods", () => {
		...
		describe("buildTransaction", () => {
			...
			it("should build bridgechain registration with default arguments", async () => {
				...
				expect(wrapper.vm.$client.buildDelegateRegistration).toHaveBeenCalledWith(
					transactionData,
					false,
					false,
				);
