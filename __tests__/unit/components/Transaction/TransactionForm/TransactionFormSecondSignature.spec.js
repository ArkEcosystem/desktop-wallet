		...
		describe("passphrase step", () => {
			...
			describe("next button", () => {
				it("should be enabled if form is valid", async () => {
					...
					expect(
						wrapper.find(".TransactionFormSecondSignature__step-2__next").attributes("disabled"),
					).toBeFalsy();
				});
				...
				it("should be disabled if form is invalid", async () => {
					...
					expect(wrapper.find(".TransactionFormSecondSignature__step-2__next").attributes("disabled")).toBe(
						"disabled",
					);
				});
			});
	...
	describe("computed", () => {
		...
		describe("passphraseWords", () => {
			it("should split passphrase by space", () => {
				...
				expect(wrapper.vm.passphraseWords).toEqual(["this", "is", "a", "passphrase"]);
			});
	...
	describe("methods", () => {
		...
		describe("buildTransaction", () => {
			it("should build second signature", async () => {
				...
				expect(wrapper.vm.$client.buildSecondSignatureRegistration).toHaveBeenCalledWith(
					transactionData,
					true,
					true,
				);
		...
		describe("displayPassphraseWords", () => {
			it("should toggle generating and show passphrase", () => {
