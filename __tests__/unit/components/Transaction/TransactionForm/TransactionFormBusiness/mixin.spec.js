...
describe.each([
	...
	describe("data", () => {
		it("should create form object", () => {
			expect(Object.keys(wrapper.vm.form)).toEqual(["fee", "passphrase", "walletPassword", "asset"]);
			expect(Object.keys(wrapper.vm.form.asset)).toEqual(["name", "website", "vat", "repository"]);
		});
	});
	...
	describe("methods", () => {
		...
		describe("buildTransaction", () => {
			it("should build business update", async () => {
				...
				if (componentName === "TransactionFormBusinessRegistration") {
					expect(wrapper.vm.$client.buildBusinessRegistration).toHaveBeenCalledWith(
						transactionData,
						true,
						true,
					);
			...
			it("should generate transaction error", () => {
				...
				if (componentName === "TransactionFormBusinessRegistration") {
					expect(wrapper.vm.$error).toHaveBeenCalledWith(
						"TRANSACTION.ERROR.VALIDATION.BUSINESS_REGISTRATION",
					);
				} else {
