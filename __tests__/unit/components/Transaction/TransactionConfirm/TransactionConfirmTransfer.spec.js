...
describe("TransactionConfirmTransfer", () => {
	...
	describe("template", () => {
		...
		it("should output senderLabel", () => {
			expect(
				wrapper.find(".TransactionConfirmTransfer__sender .ListDividedItem__value span:first-child").text(),
			).toBe("formatted-address-1");
		});

		it("should output amount", () => {
			expect(wrapper.find(".TransactionConfirmTransfer__amount .ListDividedItem__value").text()).toBe(
				"100000000",
			);
		});
		...
		it("should output recipientLabel", () => {
			expect(
				wrapper.find(".TransactionConfirmTransfer__recipient .ListDividedItem__value span:first-child").text(),
			).toBe("formatted-recipient-address");
		});

		it("should output vendorField", () => {
			expect(wrapper.find(".TransactionConfirmTransfer__vendorfield .ListDividedItem__value").text()).toBe(
				"test vendorField",
			);
		});
