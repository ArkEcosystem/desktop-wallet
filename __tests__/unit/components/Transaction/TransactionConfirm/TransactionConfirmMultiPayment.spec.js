...
describe("TransactionConfirmMultiPayment", () => {
	...
	describe("template", () => {
		...
		it("should output senderLabel", () => {
			expect(
				wrapper.find(".TransactionConfirmMultiPayment__sender .ListDividedItem__value span:first-child").text(),
			).toBe("formatted-address-1");
		});

		it("should output recipients", () => {
			const recipients = wrapper.findAll(
				".TransactionConfirmMultiPayment__recipients .InputEditableList__list__item",
			);
			...
			for (const recipientIndex in wrapper.vm.transaction.asset.payments) {
				...
				const addressText = recipientElement
					.find(".TransactionRecipientList__recipient")
					.text()
					.replace("TRANSACTION.RECIPIENT:", "");
				const amountText = recipientElement
					.find(".TransactionRecipientList__amount")
					.text()
