...
describe("TransactionConfirmBusinessRegistration", () => {
	...
	describe("template", () => {
		...
		it("should output senderLabel", () => {
			expect(
				wrapper
					.find(".TransactionConfirmBusinessRegistration__sender .ListDividedItem__value span:first-child")
					.text(),
			).toBe("formatted-address-1");
		});

		it("should output name", () => {
			expect(wrapper.find(".TransactionConfirmBusinessRegistration__name .ListDividedItem__value").text()).toBe(
				"test business",
			);
		});
		...
		it("should output website", () => {
			expect(
				wrapper.find(".TransactionConfirmBusinessRegistration__website .ListDividedItem__value").text(),
			).toBe("https://ark.io");
		});

		it("should output vat when provided", () => {
			expect(wrapper.find(".TransactionConfirmBusinessRegistration__vat .ListDividedItem__value").text()).toBe(
				"GB12345678",
			);
		});
		...
		it("should output repo when provided", () => {
			expect(
				wrapper.find(".TransactionConfirmBusinessRegistration__repository .ListDividedItem__value").text(),
			).toBe("https://github.com/arkecosystem/desktop-wallet.git");
		});
