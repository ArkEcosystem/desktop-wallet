...
describe("Services > Synchronizer > Wallets", () => {
	...
	describe("get wallets", () => {
		...
		it("should return the regular wallets and Ledger wallets", () => {
			expect(action.wallets).toIncludeSameMembers([...profileWallets, ...Object.values(ledgerWallets)]);
		});
	});
	...
	describe("get allWalletsByAdress", () => {
		beforeEach(() => {
			action.$scope.session_profile = profile;
			action.$getters["wallet/byProfileId"].mockReturnValue([profileWallets[0], profileWallets[1]]);
			action.$getters["wallet/contactsByProfileId"].mockReturnValue(contacts);
	...
	describe("fetch", () => {
		...
		beforeEach(() => {
			jest.spyOn(action, "wallets", "get").mockReturnValue([...profileWallets, ...Object.values(ledgerWallets)]);
			jest.spyOn(action, "allWallets", "get").mockReturnValue([
				...profileWallets,
				...Object.values(ledgerWallets),
				...contacts,
			...
			walletsData = [profileWallets[0], { address: Object.values(ledgerWallets)[1].address }];
	...
	describe("process", () => {
		...
		beforeEach(() => {
			...
			jest.spyOn(action, "allWallets", "get").mockReturnValue([profileWallets[0], profileWallets[1]]);
	...
	describe("processWalletsData", () => {
		...
		describe("when the data has not changed", () => {
			it("should not return them", () => {
				const data = [{ ...profileWallets[0] }, { ...Object.values(ledgerWallets)[0] }, { ...contacts[0] }];
	...
	describe("processWalletTransactions", () => {
		...
		describe("when at least 1 of them is new", () => {
			...
			it("should return the timestamp", async () => {
				expect(await action.processWalletTransactions(wallet, transactions)).toEqual(
