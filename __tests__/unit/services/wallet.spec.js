...
describe("Services > Wallet", () => {
	...
	describe("getAddress", () => {
		...
		it("should work in French", () => {
			const passphrase =
				"galerie notoire prudence mortier soupape cerise argent neurone pommade géranium potager émouvoir";
		...
		it("should work in Italian", () => {
			const passphrase =
				"mucca comodo imbevuto talismano sconforto cavillo obelisco quota recupero malinteso gergo bipede";
		...
		it("should work in Japanese", () => {
			const passphrase =
				"うかべる　くすりゆび　ひさしぶり　たそがれ　そっこう　ちけいず　ひさしぶり　ていか　しゃちょう　けおりもの　ちぬり　りきせつ";
		...
		it("should work in Korean with initially decomposed characters", () => {
			const passphrase =
				"변명 박수 사건 실컷 목적 비용 가능 시골 수동적 청춘 식량 도망";
	...
	describe("getAddressFromPublicKey", () => {
		it("should generate the correct address", () => {
			const passphrase = "one video jaguar gap soldier ill hobby motor bundle couple trophy smoke";
			const address = "DAy2xDNZLRQsgiJCnF3x4WDxGsBrmsKCsV";
			expect(WalletService.getAddressFromPublicKey(Identities.PublicKey.fromPassphrase(passphrase), 30)).toEqual(
				address,
			);
		});
	});
	...
	describe("getAddressFromMultiSignatureAsset", () => {
		it("should get address for a multisignature asset", () => {
			...
			expect(WalletService.getAddressFromMultiSignatureAsset(multisignatureAsset)).toEqual(
				"DHBKa1HFsKd9BYMPruYNAoadt5SW8oGggj",
			);
		});
	});
	...
	describe("getPublicKeyFromWallet", () => {
		it("should get public key for a standard wallet", () => {
			...
			expect(WalletService.getPublicKeyFromWallet(wallet)).toEqual(
				"037eaa8cb236c40a08fcb9d6220743ee6ae1b5c40e8a77a38f286516c3ff663901",
			);
		});
		...
		it("should get public key from multisignature info if exists", () => {
			const multiSignatureSpy = jest
				.spyOn(WalletService, "getPublicKeyFromMultiSignatureAsset")
	...
	describe("getPublicKeyFromPassphrase", () => {
		it("should get public key for a standard wallet", () => {
			...
			expect(WalletService.getPublicKeyFromPassphrase(passphrase)).toEqual(
	...
	describe("getPublicKeyFromMultiSignatureAsset", () => {
		it("should get public key from multisignature info if exists", () => {
			...
			expect(WalletService.getPublicKeyFromWallet(wallet)).toEqual(
				"03e8d4175126a39ed7ba803f31705b6f5fb78cbf46455ba778c5f39a32c6adfbd9",
			);
			expect(multiSignatureSpy).toHaveBeenCalledWith(wallet.multiSignature);
	...
	describe("isBusiness", () => {
		...
		it("should return true if wallet has ever been a business", () => {
			const wallet = {
				business: {
					name: "mybusiness",
					resigned: true,
		...
		it("should return false when checking if wallet has resigned as a business", () => {
			const wallet = {
				business: {
					name: "mybusiness",
					resigned: true,
		...
		it("should return false if wallet has never been a business", () => {
			const wallet = {};
	...
	describe("canResignBusiness", () => {
		it("should return true if business that has not resigned", () => {
			const wallet = {
				business: {
					name: "mybusiness",
					resigned: false,
		...
		it("should return false if business has resigned", () => {
			const wallet = {
				business: {
					name: "mybusiness",
					resigned: true,
		...
		it("should return false if not a business", () => {
			const wallet = {};
		...
		it("should return true for a valid passphrase", () => {
			expect(
				WalletService.isBip39Passphrase(
					"one video jaguar gap soldier ill hobby motor bundle couple trophy smoke",
					"english",
				),
			).toBeTrue();
		});
