...
describe("Services > Transaction", () => {
	...
	describe("getBytes", () => {
		it("should return the transaction bytes", () => {
			...
			expect(transactionBytes).toEqual(
				Buffer.from(
					"",
					"hex",
				),
			);
		});
	...
	describe("getHash", () => {
		...
		beforeEach(() => {
			...
			transaction = Transactions.BuilderFactory.multiSignature()
				.multiSignatureAsset({
					min: 1,
					publicKeys: [Identities.PublicKey.fromPassphrase(passphrase)],
				})
				.sign("passphrase")
				.fee(1);
		});
	...
	describe("getAmount", () => {
		...
		describe("multi-payment transaction", () => {
			let transaction;
			beforeEach(() => {
				transaction = Transactions.BuilderFactory.multiPayment()
	...
	describe("getValidMultiSignatures", () => {
		...
		beforeEach(() => {
			Managers.configManager.getMilestone().aip11 = true;
			passphrases = ["passphrase 1", "passphrase 2", "passphrase 3"];
			...
			transaction = Transactions.BuilderFactory.multiSignature().multiSignatureAsset(multiSignatureAsset).fee(1);

			transaction.data.senderPublicKey = senderPublicKey;
		});

		it("should return only valid signatures", () => {
			transaction.multiSign(passphrases[2], 2).multiSign(passphrases[0], 0).multiSign("wrong passphrase", 1);
			...
			expect(validSignatures.map((signature) => parseInt(signature.substring(0, 2)))).toIncludeSameMembers([
				0,
				2,
			]);
		});
	...
	describe("needsSignatures", () => {
		it("should check if all signatures are needed for multisig registration", () => {
			...
			const spy = jest.spyOn(TransactionService, "needsAllSignatures").mockImplementation();
		...
		it("should return true when below min required signatures", () => {
			...
			const transactionObject = Transactions.BuilderFactory.transfer()
				.amount(1)
				.fee(1)
				.recipientId(recipientAddress);
	...
	describe("needsAllSignatures", () => {
		...
		beforeEach(() => {
			...
			transactionObject = Transactions.BuilderFactory.multiSignature()
				.multiSignatureAsset(multiSignatureAsset)
				.fee(1);
	...
	describe("needsWalletSignature", () => {
		describe("multi-signature registration", () => {
			...
			it("should check if transaction is ready", () => {
				const spy = jest.spyOn(TransactionService, "isMultiSignatureReady").mockImplementation(() => false);
			...
			it("should check for final signature", () => {
				const spyReady = jest.spyOn(TransactionService, "isMultiSignatureReady").mockImplementation(() => true);

				const spyFinalSignature = jest
					.spyOn(TransactionService, "needsFinalSignature")
		...
		describe("non-registration", () => {
			...
			beforeEach(() => {
				...
				transactionObject = Transactions.BuilderFactory.transfer()
					.amount(1)
					.fee(1)
					.recipientId(recipientAddress);
			...
			it("should return true if no signatures property", () => {
				...
				expect(TransactionService.needsWalletSignature(transaction, multiSignatureAsset.publicKeys[0])).toBe(
					true,
				);
			});
			...
			it("should return false if min keys is met", () => {
				...
				expect(TransactionService.needsWalletSignature(transaction, multiSignatureAsset.publicKeys[9])).toBe(
					false,
				);
			});
		});
	...
	describe("needsFinalSignature", () => {
		...
		it("should return false if multi-signature but not registration", () => {
			transactionObject = Transactions.BuilderFactory.transfer().amount(1).fee(1).recipientId(recipientAddress);
	...
	describe("ledgerSign", () => {
		...
		it("should set the recipientId for vote transactions", async () => {
			transactionObject = Transactions.BuilderFactory.vote()
		...
		it("should not set the recipientId for bridgechain registration (type 3 group 2)", async () => {
			transactionObject = new MagistrateCrypto.Builders.BridgechainRegistrationBuilder()
		...
		it("should throw error if no signature", async () => {
			await expect(TransactionService.ledgerSign(wallet, transactionObject, vmMock)).rejects.toThrow(
				"TRANSACTION.LEDGER_USER_DECLINED",
			);
