...
describe("Services > Client", () => {
	...
	describe("fetchNetworkConfig", () => {
		const mockEndpoint = (config) => {
			nock("http://127.0.0.1").get("/api/node/configuration").reply(200, {
				data: config,
			});
		};
	...
	describe("fetchNetworkCrypto", () => {
		it("should fetch data from crypto endpoint", async () => {
			...
			nock("http://127.0.0.1").get("/api/node/configuration/crypto").reply(200, {
				data: cryptoConfig,
			});
	...
	describe("fetchFeeStatistics", () => {
		const mockEndpoint = (config) => {
			nock("http://127.0.0.1").get("/api/node/fees").query({ days: 7 }).reply(200, {
				data: config,
			});
		};
		...
		it("should return empty array on error", async () => {
			nock("http://127.0.0.1").get("/api/node/fees").query({ days: 7 }).reply(500);
	...
	describe("fetchPeerStatus", () => {
		it("should fetch data from syncing endpoint", async () => {
			...
			nock("http://127.0.0.1:4003").get("/api/node/syncing").reply(200, {
				data: response,
			});
	...
	describe("fetchDelegateVoters", () => {
		it("should fetch data from syncing endpoint", async () => {
			...
			nock("http://127.0.0.1:4003").get("/api/delegates/USERNAME/voters").query(true).reply(200, {
				meta,
				data: {},
			});
				...
				if (resource === "wallets") {
					return {
						transactions: (_, queryOptions) => {
							const transactionsResponse = transactions.filter(
								(t) => !queryOptions.type || (queryOptions.type && t.type === queryOptions.type),
							);
		...
		describe("when a custom network is specified", () => {
			it("should have the correct version", async () => {
				...
				const transaction = await client.buildTransfer(
					{ fee: new BigNumber(fees[1][0]), networkId, passphrase: "test" },
					false,
					true,
				);
	...
	describe("buildMultiSignature", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildMultiSignature(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
			...
			it("should add own wallet to signatures", async () => {
				...
				WalletService.getPublicKeyFromPassphrase = jest.fn((passphrase) =>
					Identities.PublicKey.fromPassphrase(passphrase),
				);

				const newPublicKeys = [...publicKeys, publicKey];
				...
				const signature = transaction.signatures.find(
					(s) => parseInt(s.substring(0, 2), 16) === publicKeyIndex,
				);
	...
	describe("buildIpfs", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildIpfs(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("buildMultiPayment", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildMultiPayment(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
			...
			it("should build a valid v2 transaction", async () => {
				...
				for (const recipientIndex in recipients) {
					expect(transaction.asset.payments[recipientIndex].recipientId).toEqual(
						recipients[recipientIndex].address,
					);
					expect(transaction.asset.payments[recipientIndex].amount).toEqual(
						recipients[recipientIndex].amount.toString(),
					);
				}
				expect(transaction.vendorField).toEqual(rawTransaction.vendorField);
				expect(transaction.fee).toEqual(rawTransaction.fee.toString());
	...
	describe("buildDelegateResignation", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildDelegateResignation(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("buildBusinessRegistration", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildBusinessRegistration(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("buildBusinessUpdate", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildBusinessUpdate(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("buildBusinessResignation", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildBusinessResignation(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("buildBridgechainRegistration", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildBridgechainRegistration(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("buildBridgechainUpdate", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildBridgechainUpdate(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("buildBridgechainResignation", () => {
		...
		describe("standard transaction", () => {
			it("should not build a v1 transaction", async () => {
				...
				await expect(client.buildBridgechainResignation(rawTransaction, true)).rejects.toThrow(
					"AIP-11 transaction not supported on network",
				);
			});
	...
	describe("broadcastTransaction", () => {
		beforeEach(() => {
			nock("http://127.0.0.1:4003").post("/api/transactions").reply(200, {
				data: "transaction",
			});
		});
