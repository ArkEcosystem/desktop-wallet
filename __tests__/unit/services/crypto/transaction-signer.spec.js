...
describe("Services > Client", () => {
	describe("sign", () => {
		...
		it("should default nonce to 0 if no wallet", async () => {
			const spy = setAip11AndSpy(true);
			nock("http://127.0.0.1:4003").get(`/api/v2/wallets/${address}`).reply(200, {
				data: {},
			});
		...
		describe("multiSignature", () => {
			...
			describe("own passphrase used", () => {
				...
				it("should add signature to list of signatures", async () => {
					...
					const getPublicKeyFromPassphraseMock = jest.fn((passphrase) =>
						Identities.PublicKey.fromPassphrase(passphrase),
					);
					const getPublicKeyFromPassphrase = WalletService.getPublicKeyFromPassphrase;
					...
					const signature = response.signatures.find(
	...
	describe("multiSign", () => {
		...
		it("should throw error if no passphrase or wif", async () => {
			expect(TransactionSigner.multiSign(transaction, { multiSignature })).rejects.toThrow(
				"No passphrase or wif provided",
			);
		});
		...
		it("should throw error aip11 not enabled", async () => {
			...
			expect(TransactionSigner.multiSign(transaction, signData)).rejects.toThrow(
		...
		it("should throw error if passphrase is not required for multi-signature wallet", async () => {
			expect(TransactionSigner.multiSign(transaction, { ...signData, passphrase: "not used" })).rejects.toThrow(
