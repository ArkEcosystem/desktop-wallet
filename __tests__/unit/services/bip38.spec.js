...
describe('BIP38 service', () => {
  describe('decrypt', () => {
    it('should send the data to decrypt to the worker', () => {
      ...
      expect(bip38.worker.send).toHaveBeenCalledWith({
        bip38key,
        password,
        wif
      })
    ...
    describe('when there is an error message', () => {
      it('should reject the Promise', async () => {
        await expect(
          bip38.decrypt({ bip38key: 'lol', password, wif })
        ).rejects.toBe('Failed to decrypt passphrase: Non-base58 character')
      })
    })
  })
  ...
  describe('encrypt', () => {
    ...
    describe('when there is an error message', () => {
      it('should reject the Promise', async () => {
        await expect(
          bip38.encrypt({ passphrase, password, wif: null })
        ).rejects.toBe(
