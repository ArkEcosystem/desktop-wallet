...
describe("BIP38 service", () => {
	describe("decrypt", () => {
		...
		describe("when there is a successful message", () => {
			it("should resolve the Promise", async () => {
				await expect(bip38.decrypt({ bip38key, password, wif })).resolves.toEqual({
					encodedWif: "SHPmaLNivfNRW9yf5indKKKuUCabUvNQdvpkcLJEvnNB8Q45Ki4U",
				});
			}, 20000);
		});
		...
		describe("when there is an error message", () => {
			it("should reject the Promise", async () => {
				await expect(bip38.decrypt({ bip38key: "lol", password, wif })).rejects.toBe(
					"Failed to decrypt passphrase: Non-base58 character",
				);
			});
		});
	});
	...
	describe("encrypt", () => {
		...
		describe("when there is a successful message", () => {
			it("should resolve the Promise", async () => {
				await expect(bip38.encrypt({ passphrase, password, wif })).resolves.toEqual({
		...
		describe("when there is an error message", () => {
			it("should reject the Promise", async () => {
				await expect(bip38.encrypt({ passphrase, password, wif: null })).rejects.toBe(
					"Failed to encrypt passphrase: Cannot read property 'version' of null",
				);
			});
		});
	});
