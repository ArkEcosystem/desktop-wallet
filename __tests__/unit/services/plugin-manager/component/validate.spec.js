...
describe('Validate component', () => {
  ...
  it('should have template field', () => {
    expect(validateComponent(plugin, {})).toBe(false)
    expect(validateComponent(plugin, { template: '' })).toBe(true)
  })
  ...
  it('should not have inline events', () => {
    expect(validateComponent(plugin, { template: `<input onchange="alert()" />` })).toBe(false)
    expect(validateComponent(plugin, { template: `<input onclick="alert()" />` })).toBe(false)
    expect(validateComponent(plugin, { template: `<input onfocus="alert()" />` })).toBe(false)
  })
})
