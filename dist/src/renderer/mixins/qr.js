...
export default {
    methods: {
        ...
        qr_getAddress: function (value) {
            // Possible options:
            // 1. uri
            var uri = new URIHandler(value);
            if (uri.validate()) {
                var schema = uri.deserialize();
                return schema.address;
            }
            try {
                // 2. Object { a: address } or { address: address }
                var addressObj = JSON.parse(value);
        ...
        qr_getPassphrase: function (value) {
            // Possible options:
            // 1. Object { passphrase: "passphrase words" }
            try {
                var passphraseObj = JSON.parse(value);
