	...
	<div class="PluginManager">
		<div class="px-10 py-6 mb-3 PluginManager__heading">
			<div class="flex flex-row items-center w-full">
				...
				<PluginManagerSearchBar class="ml-8 mr-auto" @search="onSearch" />
		...
		<div class="PluginManager__wrapper">
			...
			<div class="PluginManager__body">
				<div class="flex items-center">
					...
					<ButtonLayout :grid-layout="hasGridLayout" :class="{ 'ml-6': !isMenuOpen }" @click="toggleLayout" />
				...
				<template v-if="plugins.length">
					<PluginManagerGrid v-if="hasGridLayout" :plugins="plugins" @show-details="openDetailsModal" />
				...
				<div v-else class="PluginManager__body__noresults">
					<img :src="noResultsImage" class="PluginManager__body__noresults__image" />
					...
					<i18n v-else tag="span" class="mt-4 text-center" path="PAGES.PLUGIN_MANAGER.NO_RESULTS">
						...
						<span slot="category" class="inline-block font-bold">
		...
		<PluginUrlModal v-if="showUrlModal" @close="resetModal" @fetch-plugin="fetchPluginData" />
		...
		<PluginRemovalModal v-if="showRemoveModal" :plugin="selectedPlugin" @cancel="reset" @confirm="removePlugin" />
		...
		<ModalLoader :message="loadingModalText" :plugin="selectedPlugin" :visible="showLoadingModal" />
	</div>
</template>
...
export default class PluginManager extends Vue {
    ...
    onPluginInstalled(_, pluginPath) {
        try {
    ...
    onChangeStatus(enabled, pluginId) {
