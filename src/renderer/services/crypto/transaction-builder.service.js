...
export class TransactionBuilderService {
	...
	static async buildSecondSignatureRegistration(data, isAdvancedFee = false, returnObject = false) {
		return SecondSignatureRegistrationBuilder.build(data, isAdvancedFee, returnObject);
	}

	static async buildDelegateRegistration(data, isAdvancedFee = false, returnObject = false) {
	...
	static async buildDelegateResignation(data, isAdvancedFee = false, returnObject = false) {
		return DelegateResignationBuilder.build(data, isAdvancedFee, returnObject);
	}

	static async buildBusinessRegistration(data, isAdvancedFee = false, returnObject = false) {
	...
	static async buildBusinessResignation(data, isAdvancedFee = false, returnObject = false) {
		return BusinessResignationBuilder.build(data, isAdvancedFee, returnObject);
	}

	static async buildBridgechainRegistration(data, isAdvancedFee = false, returnObject = false) {
		return BridgechainRegistrationBuilder.build(data, isAdvancedFee, returnObject);
	}

	static async buildBridgechainUpdate(data, isAdvancedFee = false, returnObject = false) {
		return BridgechainUpdateBuilder.build(data, isAdvancedFee, returnObject);
	}

	static async buildBridgechainResignation(data, isAdvancedFee = false, returnObject = false) {
		return BridgechainResignationBuilder.build(data, isAdvancedFee, returnObject);
	}
}
