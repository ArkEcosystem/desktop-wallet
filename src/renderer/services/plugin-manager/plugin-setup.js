...
export class PluginSetup {
	constructor({ plugin, sandbox, vue, profileId }) {
		this.plugin = plugin;
		...
		this.pluginObject = this.sandbox
			.getComponentVM()
			.run(
	...
	__mapPermissionsToSetup() {
		return {
			...
			[PUBLIC.name]: [RegisterSetup.create(this.pluginObject), FontAwesomeSetup.create(this.plugin)],
