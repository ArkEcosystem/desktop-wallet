...
export function getSafeContext(vueContext, component) {
	...
	if (vueContext.$refs) {
		...
		const badSetters = ["href", "innerHTML", "outerHTML"];
		...
		const blockElementProperties = (element) => {
			...
			for (const badSetter of badSetters) {
				try {
					...
					throw new Error(
						`Failed to apply ${badSetter} setter to the element. Try wrapping it with '$nextTick'.`,
					);
				}
			}
			...
			for (const badGetter of badGetters) {
				try {
					...
					throw new Error(
						`Failed to apply ${badGetter} getter to the element. Try wrapping it with '$nextTick'.`,
					);
				}
			}
			...
			for (const badMethod of badMethods) {
				try {
					...
					throw new Error(
						`Failed to apply ${badMethod} method to the element. Try wrapping it with '$nextTick'.`,
					);
				}
			}
