...
class Action {
	 ...
	 */
	get profileWallets() {
		return this.profile ? this.$getters["wallet/byProfileId"](this.profile.id) : [];
	}
	 ...
	 */
	get ledgerWallets() {
		return this.$getters["session/backgroundUpdateLedger"] ? this.$getters["ledger/wallets"] : {};
	}
	...
	get contacts() {
		return this.profile ? this.$getters["wallet/contactsByProfileId"](this.profile.id) : [];
	}
	...
	async process(walletsData, transactionsByAddress) {
		...
		const addressesToUpdate = [...Object.keys(dataByWallet), ...Object.keys(walletsTransactionsAt)];
