	...
	getters: {
		...
		filtered: (_, getters, __, rootGetters) => (query, category, filter) => {
			...
			plugins = plugins.filter((plugin) => {
				...
				if (category === "all") {
					match =
						match && !plugin.config.categories.some((category) => ["theme", "language"].includes(category));
				...
				if (query) {
					match =
						match &&
						["id", "title", "description", "keywords"].some((property) => {
		...
		isGrant: (_, getters) => (pluginId) => {
			if (
				Object.prototype.hasOwnProperty.call(getters.whitelisted.global, pluginId) &&
				Object.prototype.hasOwnProperty.call(getters.whitelisted.global[pluginId], "isGrant")
			) {
	...
	actions: {
		...
		async deletePlugin({ dispatch, getters, rootGetters }, { pluginId, removeOptions = false }) {
			...
			try {
				...
				this._vm.$logger.error(`Could not delete '${pluginId}' plugin: ${error.message}`);
			}
		},
