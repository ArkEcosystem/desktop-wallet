...
export default {
	...
	getters: {
		...
		publicByProfileId: (state, _, __, rootGetters) => (profileId, getContacts = false) => {
			...
			const wallets = uniqBy([...ledgerWallets, ...profileWallets], "address");
	...
	mutations: {
		...
		UPDATE_BULK(state, wallets) {
			...
			wallets.forEach((wallet) => {
				if (profileId !== wallet.profileId) {
					throw new Error(
						`Updating wallets of different profile is not supported ('${profileId}' != '${wallet.profileId}')`,
					);
				}
		...
		DELETE(state, wallet) {
			if (state.wallets[wallet.profileId]) {
				const index = state.wallets[wallet.profileId].findIndex(
					(profileWallet) => profileWallet.id === wallet.id,
				);
				if (index === -1) {
		...
		DELETE_SIGNED_MESSAGE(state, message) {
			if (state.signedMessages[message.address]) {
				const index = state.signedMessages[message.address].findIndex(
					(signedMessage) => signedMessage.timestamp === message.timestamp,
				);
				if (index !== -1) {
