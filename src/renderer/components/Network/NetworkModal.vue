<template>
	<ModalWindow :title="title" container-classes="NetworkModal" @close="emitCancel">
		<div class="w-100">
			<InputToggle
				v-if="showFull"
		...
		<ModalLoader :message="$t('MODAL_NETWORK.VALIDATING_SEED')" :allow-close="true" :visible="showLoadingModal" />
	</ModalWindow>
</template>
		...
		if (this.network) {
			...
			if (this.network.version) {
				...
				if (networkConfig) {
					...
					try {
						ClientService.fetchNetworkConfig(this.form.server).then((network) => {
