<template>
	...
	>
		<ListDivided>
			<ListDividedItem :label="$t('TRANSACTION.ID')" item-value-class="flex items-center">
				<span
					v-tooltip="{
			...
			<ListDividedItem
				...
				item-value-class="flex items-center"
			>
				<WalletAddress :address="getAddress(transaction)" @click="emitClose" />
				<ButtonClipboard :value="getAddress(transaction)" class="text-theme-page-text-light mx-2" />
				<button
					v-tooltip="{
						content: `${$t('TRANSACTION.OPEN_IN_EXPLORER')}`,
						trigger: 'hover',
			...
			<ListDividedItem :label="$t('TRANSACTION.AMOUNT')" item-value-class="flex items-center">
				<TransactionAmount :transaction="transaction" />
				<TransactionStatusIcon v-bind="transaction" :show-waiting-confirmations="false" class="ml-2" />
			</ListDividedItem>

			<ListDividedItem :label="$t('TRANSACTION.FEE')" :value="formatter_networkCurrency(transaction.fee)" />

			<ListDividedItem v-if="transaction.nonce" :label="$t('TRANSACTION.NONCE')" :value="transaction.nonce" />

			<ListDividedItem :label="$t('TRANSACTION.TIMESTAMP')" :value="formatter_date(transaction.timestamp)" />
			...
			<ListDividedItem :value="totalCount" :label="$t('TRANSACTION.MULTI_SIGNATURE.TOTAL_SIGNATURES')" />
		</ListDivided>

		<div v-if="canSign || canBeSent" class="text-left">
			<ButtonModal
				v-if="canSign"
				slot="primaryButton"
...
export default class TransactionShowMultiSignature extends Vue {
    @Prop({
        type: Object,
        required: true,
    })
    ...
    get canSign() {
        return TransactionService.needsWalletSignature(
            this.transaction,
            WalletService.getPublicKeyFromWallet(this.wallet_fromRoute),
        );
    }
    ...
    get multiSignatureWalletAddress() {
        if (
            this.transaction.type !== TRANSACTION_TYPES.GROUP_1.MULTI_SIGNATURE ||
            !this.transaction.multiSignature
        ) {
            return null;
        }
    ...
    getAddress(transaction) {
        return (
            transaction.sender ||
            WalletService.getAddressFromPublicKey(transaction.senderPublicKey, this.session_network.version)
        );
    }
}
</script>
