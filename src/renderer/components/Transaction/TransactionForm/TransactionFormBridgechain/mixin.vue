<template>
	<form class="flex flex-col TransactionFormBridgechain" @submit.prevent>
		<template>
			...
			<ModalLoader :message="$t('MODAL_PEER.VALIDATING')" :allow-close="true" :visible="showValidatingModal" />
		</template>
	</form>
</template>
...
export default class AnonymousComponent extends Vue {
	...
	get isFormValid() {
		...
		if (this.step === 2 && this.isUpdate) {
			return (
				!this.$v.form.$invalid &&
				!(this.hasSameRepository && this.hasSameAssetRepository && this.hasSameSeedNodes && this.hasSamePorts)
			);
		}
