  ...
  methods: {
    ...
    async submit () {
      ...
      if (!this.currentWallet.isLedger) {
        transaction = await this.$client.buildVote(transactionData, this.$refs.fee.isAdvancedFee)
      } else {
        success = false
        this.showLedgerLoader = true
        try {
          const transactionObject = await this.$client.buildVote(transactionData, this.$refs.fee.isAdvancedFee, true)
