<template>
  <div
    class="TransactionDetail flex flex-row justify-between rounded-lg px-5 py-6 bg-theme-heading-background"
  >
    <div class="flex flex-row">
      <WalletIdenticon :value="senderAddress" :size="75" />
    ...
    <div class="w-48 flex justify-center items-center">
      <span v-for="n in 5" :key="n" class="text-theme-transaction-detail-arrow">
        <SvgIcon name="arrow-transaction" view-box="0 0 30 30" />
      </span>
    </div>
    <WalletIdenticon v-if="type === 0" :value="recipientAddress" :size="75" />
    <div
      v-else
      :class="[backgroundColor, textColor]"
      class="RecipientIcon flex justify-center items-center rounded-full"
    >
      <SvgIcon :name="iconName" view-box="0 0 22 22" />
    </div>
  </div>
</template>
...
.TransactionDetail {
  background-image: -webkit-linear-gradient(
    30deg,
    var(--theme-transaction-detail-gradient1) 20%,
    var(--theme-transaction-detail-gradient2) 20%
  );
  margin-left: -2rem;
  margin-right: -2rem;
}
