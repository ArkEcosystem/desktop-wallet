	...
	<div class="TransactionMultiSignatureTable w-full">
		...
		>
			<template slot-scope="data">
				<div v-if="data.column.field === 'amount'" class="flex items-center justify-end">
					<span
						v-tooltip="{
							content: `${$t('TRANSACTION.AMOUNT')}: ${formatAmount(data.row, false)}<br>${$t(
								'TRANSACTION.FEE',
							)}: ${formatFee(data.row.fee)}`,
							html: true,
				...
				<div v-else-if="data.column.field === 'senderPublicKey'" class="max-w-xxs">
					<WalletAddress
				...
				<div v-else-if="data.column.field === 'recipientId'" class="max-w-xxs">
					<WalletAddress
				...
				<div v-else-if="data.column.field === 'status'" class="max-w-xxs">
					<span v-if="needsWalletSignature(data.row)" class="text-red">
						{{ $t("TRANSACTION.MULTI_SIGNATURE.YOUR_SIGNATURE") }}
					</span>

					<span v-else-if="needsSignatures(data.row)" class="text-red">
		...
		<Portal v-if="selected" to="modal">
			<TransactionShowMultiSignature :transaction="selected" @close="onCloseModal" />
		</Portal>
	</div>
</template>
...
export default class TransactionMultiSignatureTable extends Vue {
    @Prop({
        type: Boolean,
        required: false,
        default: false,
    ...
    formatAddress(row) {
        return (
            row.sender || WalletService.getAddressFromPublicKey(row.senderPublicKey, this.session_network.version)
        );
    }
