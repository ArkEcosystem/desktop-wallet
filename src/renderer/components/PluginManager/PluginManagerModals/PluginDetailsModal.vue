<template>
	...
	>
		<template #header>
			...
			<div class="flex flex-col justify-between ml-5">
				<div>
					...
					<span class="PluginDetailsModal__header__details">
						<span>
							{{ plugin.author }}
						</span>
					</span>
				...
				<div class="PluginDetailsModal__header__actions">
					...
					<template v-else>
						...
						<ButtonIconGeneric icon="trash" view-box="0 0 14 20" class="ml-2 mr-0" @click="emitRemove" />
					</template>
					...
					<span
						...
						v-tooltip="{
							content: isBlacklisted
								? $t('PAGES.PLUGIN_MANAGER.BLACKLISTED')
								: $t('PAGES.PLUGIN_MANAGER.REPORT'),
							placement: 'bottom',
						}"
					>
						<ButtonIconGeneric
		...
		<template #default>
			...
			<p v-if="plugin.keywords.length" class="PluginDetailsModal__keywords">
			...
			<div class="mt-6">
				<SliderImage :is-row="true" :images="plugin.images">
					<template slot-scope="{ imageIndex, closeImage }">
						<SliderImageModal :images="plugin.images" :image-index="imageIndex" :close-image="closeImage" />
					</template>
				</SliderImage>
			</div>
