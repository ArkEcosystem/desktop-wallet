<template>
	<div class="w-full mt-8">
		<TableWrapper v-bind="$attrs" class="PluginManagerTable" :columns="columns" @on-sort-change="onSortChange">
			<template slot-scope="data">
				<div v-if="data.column.field === 'logo'">
					<PluginLogo :plugin="data.row" :size="40" />
				</div>

				<div v-else-if="data.column.field === 'title'" class="flex items-center">
					<span class="whitespace-no-wrap cursor-pointer" @click="emitShowDetails(data.row)">
				...
				<div v-else-if="data.column.field === 'author'" class="flex items-center">
					<span class="flex items-center whitespace-no-wrap">
						{{ data.row.author | truncate(30) }}
					</span>
				</div>

				<div v-else-if="data.column.field === 'categories'">
				...
				<div v-else-if="data.column.field === 'status'" class="flex items-center">
					<img
						v-tooltip="{
				...
				<div v-else-if="data.column.field === 'actions'" class="flex items-center justify-center">
					<ButtonIconGeneric
...
export default class PluginManagerTable extends Vue {
	@Prop({
		type: String,
		required: true,
	})
