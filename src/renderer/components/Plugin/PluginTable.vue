<template>
	<div class="PluginTable w-full">
		<TableWrapper v-bind="$attrs" class="Plugins__table" :columns="columns" @on-sort-change="onSortChange">
			<template slot-scope="data">
				<div v-if="data.column.field === 'id'" class="flex">
				...
				<div v-else-if="data.column.field === 'description'" class="flex">
					{{ data.row.description | truncate(50) }}
				</div>

				<div v-else-if="data.column.field === 'permissions'" class="flex">
					{{
						data.row.permissions && data.row.permissions.length
							? data.row.permissions.join(", ")
							: $t("PLUGIN_TABLE.NO_PERMISSIONS")
					}}
				</div>

				<div v-else-if="data.column.field === 'isEnabled'" class="flex">
