		...
		>
			<template slot-scope="data">
				<div v-if="data.column.field === 'address'" class="flex items-center">
					...
					>
						<WalletIdenticon :value="data.row.address" :size="40" class="identicon cursor-pointer" />
					</RouterLink>
				...
				<div v-else-if="data.column.field === 'name'" class="flex">
					...
					>
						...
						<span v-if="data.row.isLedger" class="ledger-badge" :class="{ 'ml-0': !walletName(data.row) }">
							{{ $t("COMMON.LEDGER") }}
						</span>
					</span>
				</div>

				<div v-else-if="data.column.field === 'multisignature'" class="flex items-center justify-center">
					<span>
						<SvgIcon
