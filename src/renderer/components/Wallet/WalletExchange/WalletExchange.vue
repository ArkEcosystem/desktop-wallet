<template>
	<section>
		<div v-if="!isPurchaseEnabled" class="mx-auto max-w-md flex flex-col items-center py-5">
			<a href="#" alt="Changelly" @click.stop="openChangellyWebsite">
				<img :src="changellyLogo" alt="Changelly" />
			</a>
			...
			<InputSwitch v-model="isChangellyEnabled" :is-reverse="true" :is-large="false">
				<i18n path="PAGES.WALLET_EXCHANGE.CHANGELLY_TERMS.CONFIRMATION" tag="div" class="ml-4 leading-normal">
					<a place="terms" href="#" @click.stop="openChangellyTerms">
						{{ $t("PAGES.WALLET_EXCHANGE.CHANGELLY_TERMS.TERMS_OF_USE") }}
					</a>
					<a place="privacy" href="#" @click.stop="openChangellyPrivacyPolicy">
						{{ $t("PAGES.WALLET_EXCHANGE.CHANGELLY_TERMS.PRIVACY_POLICY") }}
					</a>
					<a href="#" place="kyc" @click.stop="openChangellyKYC">
						{{ $t("PAGES.WALLET_EXCHANGE.CHANGELLY_TERMS.KYC") }}
					</a>
					<span place="button"> "{{ $t("COMMON.CONFIRM") }}" </span>
				</i18n>
			</InputSwitch>

			<div class="text-center mt-10">
				<button :disabled="!isChangellyEnabled" class="blue-button" @click="setConfirmed">
					{{ $t("COMMON.CONFIRM") }}
				</button>
			</div>
		</div>

		<div v-else class="WalletExchange__iframe mx-auto overflow-hidden mt-5">
			<Webview
				id="changelly"
