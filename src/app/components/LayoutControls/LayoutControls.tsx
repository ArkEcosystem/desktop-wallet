...
export const ControlButton = styled.div<{ isActive?: boolean; isChanged?: boolean }>`
	...
	${({ isChanged }) =>
		isChanged && [
			...
			css`
				&:before {
					${tw`transition-opacity duration-200 absolute inset-x-0 bg-theme-danger-400 dark:bg-theme-danger-700 rounded opacity-0 group-hover:opacity-100 w-2 h-2 rounded-full block right-0 top-0 mt-1 mr-1 left-auto`}
					content: "";
					opacity: 100;
				}
			`,
