...
describe("TransactionRowRecipientLabel", () => {
	it("should show address", () => {
		const { getByTestId } = render(<TransactionRowRecipientLabel transaction={transaction} />);
		expect(getByTestId("address__wallet-address")).toHaveTextContent("ASuusX…GQ3kqT");
	});

	it("should show label", () => {
		const { getByText } = render(
			<TransactionRowRecipientLabel transaction={{ ...transaction, type: () => "secondSignature" }} />,
		);
