...
describe("useQRCode hook", () => {
	...
	it("should generate qr code", async () => {
		...
		await act(async () => {
			await waitFor(() =>
				expect(hook.result.current.qrCodeData.uri).toBe(
			...
			await waitFor(() => expect(hook.result.current.qrCodeData.image).toBe(expectedImageData));
		});
	});
	...
	it("should generate without amount and smartbridge", async () => {
		...
		await act(async () => {
			await waitFor(() =>
				expect(hook.result.current.qrCodeData.uri).toBe("ark:D8rr7B1d6TL6pf14LgMz4sKp1VBMs6YUYD"),
			);
	...
	it("should return undefined if address is not provided", async () => {
		...
		await act(async () => {
			...
			await waitFor(() => expect(result.current.qrCodeData.uri).toBe(undefined));
			await waitFor(() => expect(result.current.qrCodeData.image).toBe(undefined));
		});
	});
});
