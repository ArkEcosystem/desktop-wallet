...
const TabStyled = styled(OptionButton)<{ isActive: boolean; isComplete: boolean; isPending: boolean }>`
	...
	${({ isPending }) =>
		isPending && tw`font-medium border-theme-primary-100 dark:border-theme-neutral-800 text-theme-primary-600`};
	min-height: 56px;
`;
...
const Tab = ({ activeTab, tabId, wordPosition }: TabProps) => {
	...
	return (
		...
		>
			{isComplete ? (
				<span className="text-lg text-theme-success-600 dark:text-theme-neutral-200">
